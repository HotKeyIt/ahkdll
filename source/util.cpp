...
#ifndef _WIN64
			,_T(" ATOM LANGID WCHAR WORD USAGE SHORT USHORT BYTE TCHAR HALF_PTR UHALF_PTR ")
#else
			,_T(" ATOM LANGID WCHAR WORD USAGE SHORT USHORT BYTE TCHAR ")
#endif
			,_T("")
#ifdef _WIN64
...
LONG WINAPI DisableHooksOnException(PEXCEPTION_POINTERS pExceptionPtrs)
{
		...
		//&& pExceptionPtrs->ExceptionRecord->ExceptionFlags == EXCEPTION_NONCONTINUABLE)
	{
		...
		{	// it is not our main process display error and exit current thread 
			...
			TCHAR *aException = (LPTSTR)_alloca(sizeof(TCHAR) * (_tcslen(Line::sSourceFile[g_script.mCurrLine->mFileIndex]) + MAX_PATH));
			_stprintf(aException, _T("Error: EXCEPTION_ACCESS_VIOLATION\n\n  -  Press yes to exit thread and continue execution.\n  -  Press no to raise exception (debug).\n  -  Press cancel to exit application.\n\nException was caused in thread id: %d\nLine: %d\nLineFile: %s"), GetCurrentThreadId(), g_script.mCurrLine->mLineNumber, Line::sSourceFile[g_script.mCurrLine->mFileIndex]);
