...
// Otherwise, aScriptFilename can be NULL if caller hasn't determined the filename of the script yet.
{
	...
	else if (!(g_hResource || (hInstance != NULL && aIsText))) // i.e. don't call the following function for stdin or loaded from resource or text.
	{
...
void Script::InitFuncLibraries(FuncLibrary aLib[])
{
	...
	// Allocate memory for WinAPI definitions, + 1 for terminator
	...
	if (SUCCEEDED(CoCreateInstance(CLSID_ShellLink, NULL, CLSCTX_INPROC_SERVER, IID_IShellLink, (LPVOID *)&psl)))
	{
		...
		if (SUCCEEDED(psl->QueryInterface(IID_IPersistFile, (LPVOID *)&ppf)))
		{
...
#endif
			{
				...
				_tcscpy(aLib[3].path, buf);
				_tcscpy(aLib[3].path + _tcslen(buf), _T("\\\0"));
			}
			ppf->Release();
		}
		psl->Release();
...
#else
	_tcsncpy(&parameterlowercase[1], aFuncName, aFuncNameLength);
