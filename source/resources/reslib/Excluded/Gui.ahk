ResolveGui(LPTSTR aBuf, LPTSTR &aCommand, LPTSTR *aName, size_t *aNameLength){
  static 
	name_marker := 0
	name_length := 0
	ConvertGuiName(aBuf, aCommand, &name_marker, &name_length)
	...
	if (!name_marker) ; i.e. no name was specified.
	{
		if (g->GuiDefaultWindowValid())
			...
			name_marker = "1" ; For backward-compatibility.
...
FindGui(aName){
  global g:=GlobalStruct()
	Loop % g.guiCount
		if g_gui[A_Index].mName = aName || g_gui[A_Index].mHwnd=aName)
			return g_gui[A_Index]
	return 0
}
...
GuiType *global_struct::GuiDefaultWindowValid()
{
	if (!GuiDefaultWindow)
	{
		...
		if (GuiDefaultWindow = GuiType::FindGui("1")) ; Assignment.
...
ResultType Script::PerformGui(LPTSTR aBuf, LPTSTR aParam2, LPTSTR aParam3, LPTSTR aParam4)
{
	...
	if (!gui.mHwnd && !gui.Create())
	{
		...
		result = ScriptError("Could not create window.")
		goto return_the_result
	}
	...
	case GUI_CMD_SUBMIT:
		result = gui.Submit(aParam2!="NoHide")
		goto return_the_result
	...
	case GUI_CMD_TAB:
	{
		...
		else
		{
			...
			if (*aParam2) ; Index or name of a particular tab inside a control.
			{
				...
				bool exact_match = aParam4="Exact" ; v1.0.37.03.
				...
				else
				{
					...
					if (index == -1)
					{
						result =ScriptError("Tab name doesn't exist yet.", aParam2)
						goto return_the_result
					}
				}
	...
	case GUI_CMD_FLASH:
		...
		FlashWindow(gui.mHwnd, aParam2!="Off" ? TRUE : FALSE)
		goto return_the_result
		...
		case GUI_CONTROL_PIC:
		{
			...
			if (*next_option == '*') ; Options are present.  Must check this here and in the for-loop to avoid omitting legitimate whitespace in a filename that starts with spaces.
			{
				...
				for ( *next_option == '*' next_option = omit_leading_whitespace(option_end))
				{
					...
					if (   !(option_end = StrChrAny(next_option, " `t"))   )  ; Space or tab.
					...
					*option_end = '\0'
					...
					if (!_tcsnicmp(next_option, "Icon", 4))
		...
		case GUI_CONTROL_DATETIME:
			...
			else ; GUICONTROL_CMD_TEXT
			{
				...
				if (*aParam3)
				{
					...
					if aParam3="LongDate" ; LongDate seems more readable than "Long".  It also matches the keyword used by FormatTime.
						style |= DTS_LONGDATEFORMAT ; Competing styles were already purged above.
					else if aParam3="Time"
...
ResultType Line::GuiControlGet(LPTSTR aCommand, LPTSTR aControlID, LPTSTR aParam3)
{
	...
	if (guicontrolget_cmd == GUICONTROLGET_CMD_FOCUS || guicontrolget_cmd == GUICONTROLGET_CMD_FOCUSV)
	{
		...
		else ; GUICONTROLGET_CMD_FOCUS (ClassNN mode)
		{
			...
			sntprintfcat(focused_control, _countof(focused_control), "%d", cah.class_count)
		}
	...
	case GUICONTROLGET_CMD_POS:
	{
		...
		if (   !(var = g_script.FindOrAddVar(var_name
			, sntprintf(var_name, _countof(var_name), "%sX", output_var.mName)))   )
		{
		...
		if (   !(var = g_script.FindOrAddVar(var_name
			, sntprintf(var_name, _countof(var_name), "%sY", output_var.mName)))   )
		{
			result = FAIL ; It will have already displayed the error.
			goto return_the_result
		}
		...
		if (   !(var = g_script.FindOrAddVar(var_name
			, sntprintf(var_name, _countof(var_name), "%sW", output_var.mName)))   )
		{
			result = FAIL ; It will have already displayed the error.
			goto return_the_result
		}
		...
		if (   !(var = g_script.FindOrAddVar(var_name
			, sntprintf(var_name, _countof(var_name), "%sH", output_var.mName)))   )
		{
			result = FAIL ; It will have already displayed the error.
			goto return_the_result
		}
...
; apply any changes to mExStyle that we make here.
{
	...
	else ; Caller is indicating that the defaults should be used.
	{
		if (*mName != '1' || mName[1]) ; Prepend the window number for windows other than the first.
			_stprintf(label_name, "%sGui", mName)
		else
			_tcscpy(label_name, "Gui")
	}
	...
	_tcscpy(label_suffix, "ContextMenu")
	...
	_tcscpy(label_suffix, "DropFiles")
...
{
	static TOO_MANY_CONTROLS "Too many controls." ; Short msg since so rare.
	...
	if (aControlType == GUI_CONTROL_TAB)
	{
		if (mTabControlCount == MAX_TAB_CONTROLS)
			return g_script.ScriptError("Too many tab controls.") ; Short msg since so rare.
	...
	{
		if (mStatusBarHwnd)
			return g_script.ScriptError("Too many status bars.") ; Short msg since so rare.
	...
	DWORD contains_bs_multiline_if_applicable =
		(opt.width != COORD_UNSPECIFIED || opt.height != COORD_UNSPECIFIED
			|| opt.row_count > 1.5 || StrChrAny(aText, "`n`r")) ; Both LF and CR can start new lines.
		...
		&& !control.jump_to_label && !(control.attrib & GUI_CONTROL_ATTRIB_IMPLICIT_CANCEL))
	{
		...
		if (*mName != '1' || mName[1]) ; Prepend the window number for windows other than the first.
			...
			*label_name = '\0'
		sntprintfcat(label_name, _countof(label_name), "Button%s", aText)
		...
		StrReplace(label_name, "`n", "", SCS_SENSITIVE)
		StrReplace(label_name, "`t", "", SCS_SENSITIVE)
		...
		case GUI_CONTROL_LINK:
		{
			...
			if (aControlType == GUI_CONTROL_LINK)
			{
				...
				for (src = aText, dst = aTextCopy *src ++src)
				{
					if (*src == '<' && ctoupper(src[1]) == 'A')
					{
						...
						if (linkText)
						{
							...
							if (LPTSTR endTag = tcscasestr(linkText, "</a>"))
							{
				...
				*dst = '\0'
				...
				draw_height = DrawText(hdc, *aTextCopy ? aTextCopy : "H", -1, &draw_rect, draw_format)
				
				delete[] aTextCopy
			}
			else
				; If no text, "H" is used in case the function requires a non-empty string to give consistent results:
				draw_height = DrawText(hdc, *aText ? aText : "H", -1, &draw_rect, draw_format)
	...
	case GUI_CONTROL_LINK:
		...
		control.hwnd = CreateWindowEx(exstyle, "SysLink", aText, style
			, opt.x, opt.y, opt.width, opt.height, mHwnd, control_id, g_hInstance, NULL)
		break
	...
	case GUI_CONTROL_DATETIME:
	{
		...
		if (*aText)
		{
			...
			if aText="LongDate" ; LongDate seems more readable than "Long".  It also matches the keyword used by FormatTime.
	...
	case GUI_CONTROL_ACTIVEX:
	{
		...
		if (!sAtlAxInitialized)
		{
			...
			if (HMODULE hmodAtl = LoadLibrary("atl"))
			{
		...
		if (control.hwnd = CreateWindowEx(exstyle, "AtlAxWin", aText, style
			, opt.x, opt.y, opt.width, opt.height, mHwnd, control_id, g_hInstance, NULL))
		{
			IObject *activex_obj
	...
	case GUI_CONTROL_CUSTOM:
		if (opt.customClassAtom == 0)
			return g_script.ScriptError("A window class is required.")
	...
	if (!control.hwnd)
		return g_script.ScriptError("Can't create control.")
...
; Caller must ensure that aOptions is a modifiable string, since this method temporarily alters it.
{
	...
	for (next_option = aOptions *next_option next_option = omit_leading_whitespace(option_end))
	{
		...
		*option_end = '\0'
		...
		bool set_owner
		if ((set_owner = !_tcsnicmp(next_option, "Owner", 5))
			  || !_tcsnicmp(next_option, "Parent", 6))
		{
			if (!adding)
				mOwner = NULL
			else
			{
				...
				if (*name || !set_owner) ; i.e. "+Parent" on its own is invalid (and should not default to g_hWnd).
				{
					...
					if (new_owner && new_owner != mHwnd) ; Window can't own itself!
						...
						return g_script.ScriptError("Invalid or nonexistent owner or parent window.", next_option)
				}
				else
		...
		else if next_option="AlwaysOnTop"
		{
		...
		else if (!_tcsnicmp(next_option, "Hwnd", 4))
			aHwndVar = g_script.FindOrAddVar(next_option + 4)

		else if (!_tcsnicmp(next_option, "Label", 5)) ; v1.0.44.09: Allow custom label prefix for the reasons described in SetLabels().
		{
			if (adding)
		...
		else if (!_tcsnicmp(next_option, "MinSize", 7)) ; v1.0.44.13: Added for use with WM_GETMINMAXINFO.
		{
			next_option += 7
			if (adding)
			{
		...
		else if (!_tcsnicmp(next_option, "MaxSize", 7)) ; v1.0.44.13: Added for use with WM_GETMINMAXINFO.
		{
		...
		else if next_option="ToolWindow"
...
; Caller must ensure that aOptions is a modifiable string, since this method temporarily alters it.
{
	...
	for (next_option = aOptions *next_option next_option = omit_leading_whitespace(option_end))
	{
		...
		*option_end = '\0'
		...
		; Attributes:
		if next_option="Section" ; Adding and removing are treated the same in this case.
			aOpt.start_new_section = true    ; Ignored by caller when control already exists.
		else if next_option="AltSubmit" && aControl.type != GUI_CONTROL_EDIT)
		{
		...
		else if (!_tcsnicmp(next_option, "Checked", 7)) ; Caller knows to ignore if inapplicable. Applicable for ListView too.
		{
			next_option += 7
			if next_option="Gray" ; Radios can't have the 3rd/gray state, but for simplicity it's permitted.
		...
		else if (!_tcsnicmp(next_option, "Choose", 6))
		{
		...
		else if next_option="VScroll" ; Seems harmless in this case not to check aControl.type to ensure it's an input-capable control.
			if (adding) aOpt.style_add |= WS_VSCROLL else aOpt.style_remove |= WS_VSCROLL
		else if (!_tcsnicmp(next_option, "HScroll", 7)) ; Seems harmless in this case not to check aControl.type to ensure it's an input-capable control.
		{
		...
		else if next_option="Tabstop" ; Seems harmless in this case not to check aControl.type to ensure it's an input-capable control.
			if (adding) aOpt.style_add |= WS_TABSTOP else aOpt.style_remove |= WS_TABSTOP
		else if next_option="NoTab" ; Supported for backward compatibility and it might be more ergonomic for "Gui Add".
			if (adding) aOpt.style_remove |= WS_TABSTOP else aOpt.style_add |= WS_TABSTOP
		else if next_option="Group" ; Because it starts with 'G', this overlaps with g-label, but seems well worth it in this case.
			if (adding) aOpt.style_add |= WS_GROUP else aOpt.style_remove |= WS_GROUP
		else if next_option="Redraw"  ; Seems a little more intuitive/memorable than "Draw".
			aOpt.redraw = adding ? CONDITION_TRUE : CONDITION_FALSE ; Otherwise leave it at its default of 0.
		else if (!_tcsnicmp(next_option, "Disabled", 8))
		{
			; As of v1.0.26, Checked/Hidden/Disabled can be followed by an optional 1/0/-1 so that
			; there is a way for a script to set the starting state by reading from an INI or registry
			; entry that contains 1 or 0 instead of needing the literal word "checked" stored in there.
		...
		else if (!_tcsnicmp(next_option, "Hidden", 6))
		{
			; As of v1.0.26, Checked/Hidden/Disabled can be followed by an optional 1/0/-1 so that
			; there is a way for a script to set the starting state by reading from an INI or registry
			; entry that contains 1 or 0 instead of needing the literal word "checked" stored in there.
		...
		else if (!_tcsnicmp(next_option, "Background"), 10))
		{
		...
		else if next_option="Group" ; This overlaps with g-label, but seems well worth it in this case.
			if (adding) aOpt.style_add |= WS_GROUP else aOpt.style_remove |= WS_GROUP
		else if next_option="Theme"
		...
		; Picture / ListView
		else if (!_tcsnicmp(next_option, "Icon", 4)) ; Caller should ignore aOpt.icon_number if it isn't applicable for this control type.
		{
			next_option += 4
			if (aControl.type == GUI_CONTROL_LISTVIEW) ; Unconditional regardless of the value of "adding".
				aOpt.listview_view = next_option!="Small" ? LVS_ICON : LVS_SMALLICON
			else
				if (adding)
					aOpt.icon_number = ATOI(next_option)
				;else do nothing (not currently implemented)
		}
		else if next_option="Report"
			aOpt.listview_view = LVS_REPORT ; Unconditional regardless of the value of "adding".
		else if next_option="List"
			aOpt.listview_view = LVS_LIST ; Unconditional regardless of the value of "adding".
		else if next_option="Tile" ; Fortunately, subsequent changes to the control's style do not pop it out of Tile mode. It's apparently smart enough to do that only when the LVS_TYPEMASK bits change.
		{
		...
		else if (aControl.type == GUI_CONTROL_LISTVIEW && next_option="Hdr")
			if (adding) aOpt.style_remove |= LVS_NOCOLUMNHEADER else aOpt.style_add |= LVS_NOCOLUMNHEADER
		else if (aControl.type == GUI_CONTROL_LISTVIEW && !_tcsnicmp(next_option, "NoSort", 6))
		{
			if SubStr(next_option,6)="Hdr" ; Prevents the header from being clickable like a set of buttons.
		...
		else if (aControl.type == GUI_CONTROL_LISTVIEW && next_option="Grid")
			if (adding) aOpt.listview_style |= LVS_EX_GRIDLINES else aOpt.listview_style &= ~LVS_EX_GRIDLINES
		else if (!_tcsnicmp(next_option, "Count", 5)) ; Script should only provide the option for ListViews.
			aOpt.limit = ATOI(next_option + 5) ; For simplicity, the value of "adding" is ignored.
		else if (!_tcsnicmp(next_option, "LV", 2))
		{
			next_option += 2
		...
		else if (!_tcsnicmp(next_option, "ImageList", 9))
		{
			if (adding)
		...
		; Button
		else if (aControl.type == GUI_CONTROL_BUTTON && next_option="Default")
			if (adding) aOpt.style_add |= BS_DEFPUSHBUTTON else aOpt.style_remove |= BS_DEFPUSHBUTTON
		else if (aControl.type == GUI_CONTROL_CHECKBOX && next_option="Check3") ; Radios can't have the 3rd/gray state.
		...
		else if next_option="Multi"
		{
		...
		else if (aControl.type == GUI_CONTROL_EDIT && next_option="WantReturn")
			if (adding) aOpt.style_add |= ES_WANTRETURN else aOpt.style_remove |= ES_WANTRETURN
		else if (aControl.type == GUI_CONTROL_EDIT && next_option="WantTab")
			if (adding) aControl.attrib |= GUI_CONTROL_ATTRIB_ALTBEHAVIOR else aControl.attrib &= ~GUI_CONTROL_ATTRIB_ALTBEHAVIOR
		else if (aControl.type == GUI_CONTROL_EDIT && next_option="WantCtrlA") ; v1.0.44: Presence of AltSubmit bit means DON'T want Ctrl-A.
			if (adding) aControl.attrib &= ~GUI_CONTROL_ATTRIB_ALTSUBMIT else aControl.attrib |= GUI_CONTROL_ATTRIB_ALTSUBMIT
		else if ((aControl.type == GUI_CONTROL_LISTVIEW || aControl.type == GUI_CONTROL_TREEVIEW)
			&& next_option="WantF2") ; v1.0.44: All an F2 keystroke to edit the focused item.
			...
			if (adding) aControl.attrib &= ~GUI_CONTROL_ATTRIB_ALTBEHAVIOR else aControl.attrib |= GUI_CONTROL_ATTRIB_ALTBEHAVIOR
		else if (aControl.type == GUI_CONTROL_EDIT && next_option="Number")
			if (adding) aOpt.style_add |= ES_NUMBER else aOpt.style_remove |= ES_NUMBER
		else if next_option="Lowercase"
		{
		...
		else if (aControl.type == GUI_CONTROL_EDIT && !_tcsnicmp(next_option, "Password", 8))
		{
		...
		else if (!_tcsnicmp(next_option, "Limit", 5)) ; This is used for Hotkey controls also.
		{
			if (adding)
			{
				next_option += 5
		...
		else if (aControl.type == GUI_CONTROL_COMBOBOX && next_option="Simple") ; DDL is not equipped to handle this style.
			if (adding) aOpt.style_add |= CBS_SIMPLE else aOpt.style_remove |= CBS_SIMPLE
		else if (!_tcsnicmp(next_option, "Sort", 4))
		{
			switch(aControl.type)
			{
			case GUI_CONTROL_LISTBOX:
			...
			case GUI_CONTROL_LISTVIEW: ; LVS_SORTDESCENDING is not a named style due to rarity of use.
				if (adding)
					aOpt.style_add |= SubStr(next_option,4)!="Desc" ? LVS_SORTASCENDING : LVS_SORTDESCENDING
				else
		...
		; UpDown
		else if (aControl.type == GUI_CONTROL_UPDOWN && next_option="Horz")
			if (adding)
			{
		...
		; Slider
		else if (aControl.type == GUI_CONTROL_SLIDER && next_option="Invert") ; Not called "Reverse" to avoid confusion with the non-functional style of that name.
			if (adding) aControl.attrib |= GUI_CONTROL_ATTRIB_ALTBEHAVIOR else aControl.attrib &= ~GUI_CONTROL_ATTRIB_ALTBEHAVIOR
		else if (aControl.type == GUI_CONTROL_SLIDER && next_option="NoTicks")
			if (adding) aOpt.style_add |= TBS_NOTICKS else aOpt.style_remove |= TBS_NOTICKS
		else if (aControl.type == GUI_CONTROL_SLIDER && !_tcsnicmp(next_option, "TickInterval", 12))
		{
			if (adding)
			{
		...
		else if (!_tcsnicmp(next_option, "Line", 4))
		{
			next_option += 4
		...
		else if (aControl.type == GUI_CONTROL_SLIDER && !_tcsnicmp(next_option, "Page", 4))
		{
			if (adding)
				aOpt.page_size = ATOI(next_option + 4)
			;else removal not supported.
		}
		else if (aControl.type == GUI_CONTROL_SLIDER && !_tcsnicmp(next_option, "Thick", 5))
		{
			if (adding)
			{
		...
		else if (!_tcsnicmp(next_option, "ToolTip", 7))
		{
			next_option += 7
		...
		else if (aControl.type == GUI_CONTROL_SLIDER && !_tcsnicmp(next_option, "Buddy", 5))
		{
			if (adding)
			{
				next_option += 5
		...
		else if (!_tcsnicmp(next_option, "Range", 5)) ; Caller should ignore aOpt.range_min/max if it isn't applicable for this control type.
		{
			if (adding)
			{
		...
		; Progress
		else if (aControl.type == GUI_CONTROL_PROGRESS && next_option="Smooth")
			if (adding) aOpt.style_add |= PBS_SMOOTH else aOpt.style_remove |= PBS_SMOOTH

		; Tab control
		else if next_option="Buttons"
		{
		...
		else if (aControl.type == GUI_CONTROL_TAB && next_option="Bottom")
			if (adding)
			{
		...
		else if (aControl.type == GUI_CONTROL_CUSTOM && !_tcsnicmp(next_option, "Class", 5))
		{
			...
			if (aOpt.customClassAtom == 0)
				return g_script.ScriptError("Unregistered window class.", className)
		}

		; Styles (alignment/justification):
		else if next_option="Center"
			if (adding)
			{
				switch (aControl.type)
				{
		...
		else
		{
			...
			case 'V': ; Variable
				...
				if (candidate_var->IsNonStaticLocal()) ; Note that an alias can point to a local vs. global var.
					return g_script.ScriptError("A control's variable must be global or static.", next_option - 1)
...
ResultType GuiType::Show(LPTSTR aOptions, LPTSTR aText)
{
	...
	for (LPTSTR cp_end = aOptions, cp = aOptions *cp cp = cp_end)
	{
		...
		case 'C':
			...
			break
		case 'M':
			if (!_tcsnicmp(cp, "Minimize", 8)) ; Seems best to reserve "Min" for other things, such as Min W/H. "Minimize" is also more self-documenting.
			{
				; Skip over the text of the name so that it isn't interpreted as option letters.
				cp_end += 8
				show_mode = SW_MINIMIZE  ; Seems more typically useful/desirable than SW_SHOWMINIMIZED.
			}
			else if (!_tcsnicmp(cp, "Maximize", 8))
			{
				; Skip over the text of the name so that it isn't interpreted as option letters.
				cp_end += 8
			...
			break
		case 'N':
			if (!_tcsnicmp(cp, "NA", 2))
			{
				; Skip over the text of the name so that it isn't interpreted as option letters.
				cp_end += 2
				show_mode = SW_SHOWNA
			}
			else if (!_tcsnicmp(cp, "NoActivate", 10))
			{
				; Skip over the text of the name so that it isn't interpreted as option letters.
				cp_end += 10
...
ResultType GuiType::ControlGetContents(Var &aOutputVar, GuiControlType &aControl, LPTSTR aMode)
{
	...
	if aMode!="Text" ; Non-text, i.e. don't unconditionally use the simple GetWindowText() method.
	{
	...
	else if (aControl.type == GUI_CONTROL_EDIT) ; Auto-translate CRLF to LF for better compatibility with other script commands.
	{
		...
		; of CRLFs:
		StrReplace(aOutputVar.Contents(), "`r`n"), "`n", SCS_SENSITIVE)
...
; be added to the array.
{
	...
	for (LPTSTR cp = aOptions *cp ++cp)
	{
		...
		case 'C': ; Color
			...
			if (space_pos = StrChrAny(color_str, " `t"))  ; space or tab
				*space_pos = '\0'
	...
	{
		g_script.ScriptError("Too many fonts.")  ; Short msg since so rare.
		return -1
	}
	...
	{
		g_script.ScriptError("Can't create font.")  ; Short msg since so rare.
		return -1
	}
...
int GuiType::FindFont(FontType &aFont)
{
	for (int i = 0 i < sFontCount ++i)
		if (sFont[i].name=aFont.name ; lstrcmpi() is not used: 1) avoids breaking existing scripts 2) provides consistent behavior across multiple locales.
...
bool GuiType::ConvertAccelerator(LPTSTR aString, ACCEL &aAccel)
{
	...
	while (cp = _tcschr(aString + 1, '+')) ; For each modifier.  +1 is used in case "+" is the key.
	{
		...
		if (!_tcsnicmp(aString, "Ctrl", len))
			modLR |= MOD_LCONTROL
		else if (!_tcsnicmp(aString, "Alt", len))
			modLR |= MOD_LALT
		else if (!_tcsnicmp(aString, "Shift", len))
			modLR |= MOD_LSHIFT
		else
			return false
