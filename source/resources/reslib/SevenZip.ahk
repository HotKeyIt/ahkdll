Class SevenZip{
  ...
  __New(hwnd:=0){
    static DefaultSevenZipError:=(SevenZip.SevenZipError.Default:="ERROR_UNKNOWN_ERROR"),dll:=UnZipRawMemory(LockResource(LoadResource(0,hRes:=DllCall("FindResource","PTR",0,"Str","556EA2A65AE54D58BC52C792B3ED2ED0","PTR",10,"PTR"))),SizeofResource(0,hRes))
    this.lib:=lib:=MemoryLoadLibrary(dll.Ptr,dll.size)
  ...
  ExtractMem(archive, file:="", options:=""){
    ...
    if Err:=this._ExtractMem(this.hwnd,StrBuf("x " options " `"" archive "`"" (file ? "`"" file "`"":""),"UTF-8"),buf,szBuf,getvar(time:=0),getvar(attr:=0),getvar(szWrite:=0))
  ...
  ExtractMemEx(archive, file:="", options:=""){
    ...
    this.CloseArchive(harc)
    buf:=Buffer(szBuf:=info.dwOriginalSize)
    if Err:=this._ExtractMemEx(this.hwnd,StrBuf("x " options " `"" archive "`"" (file ? "`"" file "`"":""),"UTF-8"),buf,szBuf,getvar(time:=0),getvar(attr:=0),getvar(szWrite:=0))
