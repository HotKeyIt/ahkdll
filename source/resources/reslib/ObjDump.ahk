...
RawObject(obj,addr,buf:=0){
  ...
  { ; 9 = Int64 for size and Char for type
    If IsObject(k)
			NumPut(-11,addr+0,"Char"),NumPut(sz:=RawObjectSize(k,buf),addr+1,"Int64"),RawObject(k,addr+9,buf),addr+=sz+9
    else if (k+0="")
			NumPut(-10,addr+0,"Char"),NumPut(sz:=StrPut(k,addr+9)*(A_IsUnicode?2:1),addr+1,"Int64"),addr+=sz+9
    else 
