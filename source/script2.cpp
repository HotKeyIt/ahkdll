		...
		if (aResultToken.value_int64)
		{
			...
			if (!ParamIndexIsOmitted(2))
			{
				if (aParam[2]->symbol == SYM_VAR)
				{
					...
					*(aParam[2]->var->mCharContents + aResultToken.value_int64 / sizeof(TCHAR)) = '\0';
				}
...
MsgMonitorStruct *MsgMonitorList::Find(UINT aMsg, HWND aHwnd, IObject *aCallback)
{
	for (int i = 0; i < mCount; ++i)
		if (mMonitor[i].msg == aMsg && mMonitor[i].hwnd == aHwnd && mMonitor[i].func == aCallback)
			return mMonitor + i;
	return NULL;
}
