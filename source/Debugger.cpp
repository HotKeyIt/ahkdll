...
int Debugger::PreExecLine(Line *aLine)
{
	...
	if (InterlockedIncrement(&mInterlockedExec) > 1){
		...
		while (mInterlockedExec || mInternalState == DIS_Break 
				|| mInternalState == DIS_StepInto || mInternalState == DIS_StepOver 
...
DEBUGGER_COMMAND(Debugger::breakpoint_set)
{
	...
	for (int i = 0; i < MAX_AHK_THREADS; i++)
	{
		...
		if (filename)
		{
			...
			CStringTCharFromUTF8 filename_t(filename);
			// Find the specified source file.
			int SourceFileCount = (int)g_ahkThreads[i][3];
			LPTSTR *SourceFile = (LPTSTR*)g_ahkThreads[i][2];
			for (file_index = 0; file_index < SourceFileCount; ++file_index)
				if (!_tcsicmp(filename_t, SourceFile[file_index]))
				{
...
DEBUGGER_COMMAND(Debugger::breakpoint_get)
{
	...
	for (int i = 0; i < MAX_AHK_THREADS; i++)
	{
		if (!IsWindow((HWND)g_ahkThreads[i][0]))
			continue;
		for (line = ((Script*)g_ahkThreads[i][1])->mFirstLine; line; line = line->mNextLine)
		{
			if (line->mBreakpoint && line->mBreakpoint->id == breakpoint_id)
			{
				mResponseBuf.WriteF("<response command=\"breakpoint_get\" transaction_id=\"%e\">", aTransactionId);
		...
		for (line = ((Script*)g_ahkThreads[i][1])->mFirstLine; line; line = line->mNextLine)
		{
			...
			if (bp && bp->id == breakpoint_id)
			{
				if (lineno && line->mLineNumber != lineno)
				{
					...
					if (!line)
					{
						result = DEBUGGER_E_BREAKPOINT_INVALID;
						break;
					}
	...
	for (se = mStack.mTop; se >= mStack.mBottom; --se)
	{
		if (depth == -1 || depth == level)
		{
			...
			if (se == mStack.mTop)
			{
				...
				ASSERT(mSourceFile); // Should always be valid.
...
#ifdef _WIN64
				aDebugger->WriteProperty(_i64toa(i + 1,intbuf,10), value);
#else
				aDebugger->WriteProperty(_itoa(i + 1,intbuf,10), value);
#endif
			}
		}
		else
...
int Debugger::ParsePropertyName(const char *aFullName, int aVarScope, bool aVarMustExist, Var *&aVar, Object::FieldType *&aField)
{
	...
	if ( !var->HasObject() || !(obj = dynamic_cast<Object *>(var->Object())) 
						   && !(criobj = dynamic_cast<CriticalObject *>(var->Object())) 
						   && !(strct = dynamic_cast<Struct *>(var->Object())) )
		return DEBUGGER_E_UNKNOWN_PROPERTY;
	...
	for (*name_end = c; ; )
	{
		...
		if ( (obj && !(field = obj->FindField(key_type, key, insert_pos))) )
		{
		...
		else if (!obj)
		{
			...
			// destructor to be called):
			static Object::FieldType *sBaseField = new Object::FieldType();
			static Var *aVar = (Var*)malloc(sizeof(Var));
			_thread_local static TCHAR buf[MAX_PATH];
			...
			if (criobj)
				criobj->Invoke(aResultToken,aThisToken,0,&aVarToken,1);
			else
				strct->Invoke(aResultToken,aThisToken,0,&aVarToken,1);
			...
			switch (sBaseField->symbol = aResultToken.symbol)
			{
				case SYM_OBJECT:
					...
					{
						criobj = NULL;
						criobj = dynamic_cast<CriticalObject *>(aResultToken.object);
						if (!criobj) // criobj failed, try obj
						{
							obj = dynamic_cast<Object *>(aResultToken.object);
							if (!obj) // obj failed, try struct
...
LPTSTR Var::ObjectToText(LPTSTR aBuf, int aBufSize)
{
	...
	IObject *aObject = ToObject();
	aBuf += sntprintf(aBuf, aBufSize, _T("%s: Object(0x%p)"), mName, aObject);
	if (ComObject *cobj = dynamic_cast<ComObject *>(aObject))
